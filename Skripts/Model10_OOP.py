...
class ConvModel(BaseModel):
    def build_model(self):
        ...
        x1 = layers.Conv2D(filters=6, kernel_size=(4, 3), padding='valid', kernel_regularizer=l2(self.reg), 
                           activation='relu')(x['X_angles'])
        x1 = layers.Reshape((self.config['data']['window_size'], -1))(x1)
        x1 = layers.Conv1D(filters=16, kernel_size=2, padding='valid', dilation_rate=1, kernel_regularizer=l2(self.reg), activation='relu')(x1)
        x1 = layers.Conv1D(filters=64, kernel_size=2, padding='valid', dilation_rate=2, kernel_regularizer=l2(self.reg), activation='relu')(x1)
        x1 = layers.Conv1D(filters=64, kernel_size=2, padding='valid', dilation_rate=4, kernel_regularizer=l2(self.reg), activation='relu')(x1)
        x1 = layers.Conv1D(filters=64, kernel_size=2, padding='valid', dilation_rate=8, kernel_regularizer=l2(self.reg), activation='relu')(x1)
        # if self.max_pooling:
        #     x1 = layers.MaxPool1D(pool_size=3, strides=None, padding='valid')(x1)

        x2 = layers.Conv1D(filters=16, kernel_size=2, padding='valid', dilation_rate=1, kernel_regularizer=l2(self.reg), activation='relu')(x['X_pointquat'])
        x2 = layers.Conv1D(filters=64, kernel_size=2, padding='valid', dilation_rate=2, kernel_regularizer=l2(self.reg), activation='relu')(x2)
        x2 = layers.Conv1D(filters=64, kernel_size=2, padding='valid', dilation_rate=4, kernel_regularizer=l2(self.reg), activation='relu')(x2)
        x2 = layers.Conv1D(filters=64, kernel_size=2, padding='valid', dilation_rate=8, kernel_regularizer=l2(self.reg), activation='relu')(x2)
        # if self.max_pooling:
